...
anim = @animate for i=1:length(snaps)
    ...
    w = [ev.requestedEnergy - ev.currentWorkload for ev in sim.snapshots[i].charging];
    d = [ev.departureTime - ev.arrivalTime - ev.currentDeadline for ev in sim.snapshots[i].charging];
    ...
    xlabel!(p,"Carga alcanzada");
    ylabel!(p,"Tiempo transcurrido");
    title!(p,"Población EV")
