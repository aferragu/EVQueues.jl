...
sim = ev_fifo(lambda,mu,gamma,Tfinal,C,snapshots=[Tfinal])
...
#Attained vs. requested

Sa = [ev.departureWorkload for ev in sim.EVs][30000:end];
S = [ev.requestedEnergy for ev in sim.EVs][30000:end];

p = plot(   xlabel="Requested",
            ylabel="Attained",
            title="Attained work vs. requested work")

scatter!(S,Sa,markershape=:circle,markersize=4,color=:blue,legend=:none)
display(p)
...
d = snap.t.-[ev.arrivalTime for ev in snap.charging];
on = [ev.currentPower>0 for ev in snap.charging];

p = plot(   xlabel = "Attained workload",
            ylabel = "Soj. time",
